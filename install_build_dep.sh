...
# t64 transition: noble (Ubuntu 24.04+) and trixie (Debian 13+) renamed many
# Qt5 binary packages with a "t64" suffix for the time_t 64-bit ABI change.
# Older distros (jammy, focal, bullseye, bookworm) keep the original names —
# detect and rewrite the package list so a single hardcoded set works on both.
_codename="$(. /etc/os-release 2>/dev/null && echo "${VERSION_CODENAME}")"
_t64_rewrite_sed=""
case "$_codename" in
    jammy|focal|bullseye|bookworm)
        _t64_rewrite_sed='s/libqt5core5t64/libqt5core5a/g; s/libqt5([a-z]+)5t64/libqt5\15/g' ;;
esac
